...
//		test(Constants.MAI_SHOU_DANG);
	...
	public static void fetchGoodByRule(int merType, String classValue){
		...
		case Constants.GUANG_DIU:
			...
			dfr.setDescCssStyle("div[id=dabstract]#p,1");//p标签的深度控制--
		...
		case Constants.SHENME_ZHIDE_MAI:
			...
			dfr.setTitleCssStyle("h1");//里的h1标签#h1,[class=article_title]--
			dfr.setDescCssStyle("div[class=item-box item-preferential]#p,2");//p标签的深度控制,外层class=ht-deal-detail-des-box--
			dfr.setTrueLinkCssStyle("div[class=buy]#link,rel=canonical");//外层class=ht-deal-detail-buy--
		...
		case Constants.WUWU_HAITAO:
			...
			dfr.setDescCssStyle("div[class=ht-deal-detail-des-box]#p,1");//p标签的深度控制,外层class=ht-deal-detail-des-box--
			dfr.setTrueLinkCssStyle("a[class=ht-deal-detail-buy-btn]#link,rel=canonical");//外层class=ht-deal-detail-buy--
		...
		case Constants.MEI_DE_BI:
			...
			dfr.setDescCssStyle("div[id=infoDesc]#p,1");//p标签的深度控制--
		...
		case Constants.BEI_MEI_SHENG_QIAN_KUAI_BAO:
			...
			dfr.setDescCssStyle("div[class=minfor  event_statistics]#p,1");//li标签的深度控制//--
			...
			if(getDesc){
				...
				if (mainContent != null && mainContent.size() > 0) {
					...
					for (int i = 0; i < subContents.size(); i++) {
						...
						//TODO 没得比,这里需要控制p标签的深度,有的描述要有三四个p标签
						//TODO 北美省钱快报,这里需要控制li标签的深度,默认只取一个li标签
						if(i > depth){
							break;
						}
...
//						System.out.println("类型节点"+typeCssArr[0]+"有"+types.size()+"个元素");
						...
						// TODO 获取类型暂时只设置默认为6
						Elements type = types.select(aTag);//没得比:"span");//55海淘get(0).getElementsByClass("ht-deal-detail-info-tag");
						Elements realTypes =type;// 没得比: type.get(3).select("a");//没得比操作
						System.out.println("realTypes:"+realTypes.text());
						Element realType = realTypes.get(depth);//没得比 realTypes.get(0);//55海淘last();
						// 获取类型
...
//				System.out.println("getGoodSrcLinkByJsoup#goodSrcLink(真实链接):"+goodSrcLink+", startWithHTTP:"+goodSrcLink.startsWith("http://")+", startWithHTTP:"+goodSrcLink.startsWith("https://"));
//			}
...
//		goodList = fetchGoodLink(rule);
		 ...
		 * cate.php?m=Amazon&k=zdirect&c=us cate.php?m=Amazon&k=digital&c=us
		 * cate.php?m=Amazon&k=electrical&c=us cate.php?m=Amazon&k=makeup&c=us
		 * cate.php?m=Amazon&k=clothes&c=us cate.php?m=Amazon&k=clothes&c=us
		 * cate.php?m=Amazon&k=daily&c=us cate.php?m=Amazon&k=food&c=us
		 * cate.php?m=Amazon&k=baby&c=us cate.php?m=Amazon&k=automobile&c=us
